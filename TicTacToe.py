...
def players_turn(board):
    try:
        move = int(input("Enter the number of position at which you want to play."))
        move = move - 1
        col = move%3
        row = move//3
        if(board[row][col]=='-' and move>=0):
            ...
            print("Please select a bank Space ")
            players_turn(board)
    except (IndexError,ValueError):
        print("Enter a number in between 0-9")
        players_turn(board)
    player_turn = False
...
def turn2():
    ...
    while turn != 'Y' and turn != 'N':
        try:
            turn = input("Press Y to Go first else N.")
            turn = turn.upper()
            if(turn=='Y'):
                player_turn = True
            else:
...
def play_game():
    ...
    human,ai = move2()
    player_turn = turn2()
    print("Below positions number are shown as you want to move.") 
    for i in board:
        for j in i:
            print(temp,end='\t')
            temp += 1
